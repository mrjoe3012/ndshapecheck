...
def test_parse_shape_str() -> None:
    symbols, literals = _parse_shape_str('1, 1?, 15')
    assert symbols == ['1', '1?', '15']
    assert literals == [1, 1, 15]
    symbols, literals = _parse_shape_str('N?, A, myname_is_joe*, joe_is12_my_NAME+, B')
    assert symbols == ['N?', 'A', 'myname_is_joe*', 'joe_is12_my_NAME+', 'B']
    assert literals == [None, None, None, None, None]
    symbols, literals = _parse_shape_str('N?, 1, 2, joe_is_MY_name, 3')
...
def test_construct_rule_regex() -> None:
    symbols = ['N+', '3']
    literals = [None, 3]
    regex = _construct_rule_regex(symbols, literals)
